package com.example.demo.service;

import java.util.List;
import java.util.Optional;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Service;

import com.example.demo.model.ProductEntity;
import com.example.demo.repository.ProductRepo;

@Service
public class ProductService {

	@Autowired
	ProductRepo rep;
	
	// to add Single detail
	public String addinfo(ProductEntity pro)
	{
		if(rep.existsById(pro.getProduct_id()))
		{
			return "Duplication of Id not allowed";
		}
		else
		{
			rep.save(pro);
			return "Data added Successfully";
		}
	}
	
	// to add list of details
	public String addninfo(List<ProductEntity> pro)
	{
		if(rep.existsById(pro.get(0).getProduct_id()))
		{
			return "Duplication of Id not allowed";
		}
		else
		{
			rep.saveAll(pro);
			return "All Data's added successfully";
		}
	}
	
	// to display all details
	public List<ProductEntity> showinfo(){
		return rep.findAll();
	}
	
	// to display the specific detail by id
	public Optional<ProductEntity> showid(int id) {
		return rep.findById(id);
	}
	
	// to update changes in database
	public String changeinfo(ProductEntity pro) {
		if(rep.existsById(pro.getProduct_id()))
		{
			rep.saveAndFlush(pro);
			return "Updated Successfully";
		}
		else
		{
			return "Invalid request for updation";
		}
	}
	
	// to update by id in database
	public String changeinfoid(int id,ProductEntity pro) {
		if(rep.existsById(id))
		{
			rep.saveAndFlush(pro);
			return "Updated Successfully";
		}
		else
		{
			return "Invalid request for updation";
		}
	}
	
	// to delete an row by specifying the whole data
	public String deleteinfo(ProductEntity pro) {
		if(rep.existsById(pro.getProduct_id()))
		{
			rep.delete(pro);
			return "Deleted Successfully";
		}
		else
		{
			return "Invalid request for deletion";
		}
	}
	
	// to delete by specifying the id
	public String deleteinfoid(int id) {
		if(rep.existsById(id))
		{
			rep.deleteById(id);
			return "Deleted Successfully";
		}
		else
		{
			return "Invalid request for deletion";
		}
	}
}